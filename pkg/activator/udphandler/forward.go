...
func Forward(src, dst string, t Throttler, ctx context.Context, timeout time.Duration) (*Forwarder, error) {
	...
	forwarder.throttler = t
	forwarder.timeout = timeout
	udpNamespacedName := new(types.NamespacedName)
	udpNamespacedName.Namespace = "default"
	udpNamespacedName.Name = "danitag-echo-gw9th"
	log.Printf("udpNamespacedName: ", udpNamespacedName)
	testCtx := context.Background()
	testCtx = util.WithRevID(testCtx, *udpNamespacedName)
	...
	go forwarder.run(t, testCtx)

	return forwarder, nil
}

func (f *Forwarder) run(t Throttler, ctx context.Context) {
	for {
		buf := make([]byte, bufferSize)
		oob := make([]byte, bufferSize)
		log.Printf("Now calling ReadMsgUDP")
		n, _, _, addr, err := f.listenerConn.ReadMsgUDP(buf, oob)
		log.Printf("ReadMsgUDP called")
		if err != nil {
			log.Println("forward: failed to read, terminating:", err)
			return
		}
		//log.Printf("Run buffer: ", buf)
		log.Printf("N count: ", n)
		log.Printf("addr value: ", addr)
		t.Try(ctx, func(dest string) error{
